...
def put_text(img, text, position, font_size=24, color=(0, 255, 0)):
...
def calculate_avoidance_maneuver(frame, results, danger_center_ratio=0.2):
    ...
    if not obstacles_in_danger_zone:
        return "Course ahead clear. Move forward."
    ...
    if len(left_obstacles) < len(right_obstacles):
        ...
        maneuver = "Turn left." if avg_left_y > avg_right_y else "Turn right."
    return maneuver
...
def run_avoidance_system(video_source=0):
    ...
    while cap.isOpened():
        ...
        frame = put_text(frame, command, (30, 30))
